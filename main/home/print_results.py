...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print("% Match: ", results_stats_dic['pct_match']) #(optional - this includes both dogs and not-a dog)
    
#     for i in results_stats_dic:
#         print(i, ": ", results_stats_dic[i]) #Can use switch using for loop
        ...
        if(results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'] == results_stats_dic['n_images']):
            ...
            for i in results_dic:
                if(sum(results_dic[i][3:])==1):
                    count += 1
                    if count ==1:
                        print("\nMisclassification of dogs:")
                        
                    #print("Misclassified image: ", count) #Commented as the following line might cause confusion in output
                    print("Pet Image Label: ", results_dic[i][0])
                    print("Classifier Label: ", results_dic[i][1])
                    print()
            print("Total dog/not-dog misclassifications: ", count)  
    ...
    if print_incorrect_breed:
        if(results_stats_dic['n_correct_dogs'] == results_stats_dic['n_correct_breed']):
            ...
            count = 0
            for i in results_dic:
                if(sum(results_dic[i][3:])==2 and results_dic[i][2]==0):
                    count += 1
                    if count ==1:
                    ...
                    print("Classifier Label: ", results_dic[i][1])
                    print()
            print("Total dog breed misclassifications: ", count)  